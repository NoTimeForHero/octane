...
export async function validateTransaction(
    ...
    if (!fee.value) throw new Error('blockhash not found');
    //console.log('fee', fee.value, 'expected', lamportsPerSignature);
    if (fee.value > lamportsPerSignature * 3) throw new Error('fee too high');
